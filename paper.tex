...
Another approach to ECPS modeling concerns quantized control systems and finite-word length (FWL) effects on the performance of these systems. Differently from bisimulations, quantized models are built from a countable subset of the (quantized) input space~\cite{Tabuada07}. In particular, Keel and Bhattacharyya showed that even the robust and optimal controllers may still present an undesired property~\cite{Keel01,bhattacharyya}: fragility or sensitivity to implementation aspects ({\it e.g.}, the FWL representation of the digital controllers).  Therefore, various works tried link sampled-data theory to hybrid systems models~\cite{Bicchi02,Petreczky10,Petreczky07,Ye98}.

Finally, in addition to the modeling of the ECPS dynamics itself, it is often interesting to establish invariants on the state space of the ECPS, such information can be used in order to accelerate formal verification and synthesis processes for these systems. These invariants might be synthesized in order to point the boundaries of state-space~\cite{BenSassi12}, determine stability~\cite{Paul14}, or reachability of ECPS~\cite{Li16_2,Li16}.
...
Generally, the properties that must hold for ECPSs are related to safety/reachability, and liveness. Therefore, model-checking procedures have been employed to verify those properties even under undecidability conditions via approximations, {\it e.g.}, bounding the number of steps from the initial state, which leads to bounded model checking (BMC)~\cite{Veanes09}. 

Along with the CPS modeling and verification techniques purely based on HA, appeared logic-based approaches~\cite{Phan15} that may still use HA, but it is not based on a algebraic description of states and transitions. Some examples of logic-based verification are the theorem proving techniques, {\it e.g.} deductive verification (i.e., interactive theorem proving)~\cite{Nipkow02} and automatic theorem proving~\cite{Platzer07}, and reduction-based techniques, among them the BMC stands out~\cite{handbook09}.
...
Recently, some of these works addressed the ECPS formal verification considering the discretization, switching and quantization effects.  Dugirala and Viswanathan~\cite{Duggirala2015} verified embedded control softwares considering real time issues, e.g., delayed responses and possible loss of real time deadlines. Anta {\it et al.}~\cite{Anta10} presented a tool called Costan that finds errors in the implementation of a mathematical model and verifies whether the error is tolerated, considering the quantization effects and the fixed-point implementation focusing on the quantization error effects over stability of the system. Similarly, Ismail {\it et al.}~\cite{dsv_spin2015} presets the DSVerifier, a BMC tool for digital systems that employs SMT/SAT solvers as back-ends in order to provide support for digital system design and verification considering FWL effects. In particular, DSVerifier verifies if a designed digital controller and/or closed-loop ECPS presents the desired performance when it is implemented in a FWL format~\cite{Bessa16,Bessa17}.

Finally, it is worth to mention that there are some other alternatives for modeling and analysis of ECPS, that are not based on verification, but in simulators~\cite{Phan2014,Nakajima12,Junjie12}. These approaches fails to ensure that some properties are hold but it presents a good scalability, what is ideal for large scale and complex systems that are hardly modeled through the automata- or logig-based approaches. Modelica~\cite{Junjie12} and Simulink/MATLAB~\cite{Nakajima12} are important examples of widely used simulators for CPS. In order to achieve a trade off between the scalability of CPS simulators and the correctness of verification methods, the statistical model checking have been employed to formally verify ECPS~\cite{Clarke11}.


In addition, Shoukry {\it et al.} \cite{Shoukry} tackled another important problem related to ECPSs: security. Indeed, the essence of ECPSs claims for sophisticated interfacing ({\it e.g.}, sensors and network connections), which normally leads to security issues~\cite{cpsattack}, given that wrong control actions can be generated through corrupted measurements. The mentioned authors proposed a methodology for estimating the state of physical systems, through formal methods, even if sensor inputs are contaminated with erroneous or intentionally corrupted data, in order to still be able to execute suitable control commands. The cyber-security is also a concern of Fiore {\it et al.} that aims to ensure the secure state estimation of UAV systems under sparse attacks~\cite{Fiore17}. Indeed, many situations can be regarded as satisfiability problems and solved, for instance, with approaches based on Satisfiability Modulo Theories (SMT)~\cite{Araujo16,TrindadeDAES2016,Rahman13}, which indicates many links with other research areas and might lead to a unified approach regarding system verification, security enforcement, and interaction handling.
...
The control synthesis for ECPS consists of finding an controller or an invariant control law $\mathcal{C}$, such that a closed-loop system composed of $(\mathcal{C},\Omega)$ satisfies every property $\mathcal{L}\in \Phi$. 

An efficient way to synthesize reliable ECPS is the Counterexample Guided Inductive Synthesis (CEGIS)~\cite{David15}. The key idea is the definition (by the user) of the safe states set, {\it i.e.} the specification of $\Phi$, as well as a generic template for a control system that must satisfy these specification; the the CEGIS engine compute values for the open values in the template that hold $\Phi$~\cite{RienerKFB16}. Abate~{\it et al.}~\cite{Abate17,abatecav2017} presents a method for synthesizing stable controllers that are suitable to continuous plants given as transfer functions~\cite{Abate17} and state-space representations~\cite{abatecav2017}, which exploits bit-accurate verification of software implemented in digital microcontrollers~\cite{Bessa16,Bessa17,dsv_spin2015}.
