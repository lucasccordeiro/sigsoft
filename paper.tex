...
\title{Automated Verification and Synthesis of Embedded \& Cyber-Physical Systems: Advances and Challenges}
...
\textcolor{red}{confesso que, para mim, o paragrafo abaixo ficou estranho... parece que estamos misturando tudo... sera que na tengtativa de implementar os comentarios e ter o artigo aceito nao estamos deixando o artigo muito nebuloso?}
...
For instance, consider the C-code fragment shown in Figure~\ref{figure:inductive-invariant}; \textcolor{blue}{the star-notation indicates nondeterminism. Suppose that one wants to prove that $P : x > 0$ is invariant. To attempt to prove the invariant property P, one can apply induction:}
...
\begin{itemize} 
\item \textcolor{blue}{Whenever $P$ holds for $k$ loop unwindings, it also holds for $k + 1$ steps.}
\end{itemize}
...
\textcolor{blue}{Bradley {\it et al.} introduced  ``property-based reachability'' (or IC$3$) procedure for the safety verification of systems~\cite{Bradley12,Bradley13}; the authors have shown that IC$3$ can scale on certain benchmarks, where \textit{k}-induction fails to succeed. In particular, the success of IC3 over \textit{k}-induction procedures is due to the ability of the former to guide the search for inductive instances with counterexamples to the inductiveness (CTIs) of the given property~\cite{Bradley18}. Consider again the C-code fragment shown in Figure~\ref{figure:inductive-invariant}, that $P : x > 0$ is an invariant.}
...
\textcolor{blue}{Jovanovi\'{c} {\it et al.}~\cite{Jovanovic:2016:PK:3077629.3077648} present a reformulation of IC3, separating the reachability checking from the inductive reasoning. They further replace the regular induction algorithm by the \textit{k}-induction algorithm and show that it provides more concise invariants. The authors implemented the algorithm in the SALLY model checker using Yices2 to do a forward search and MathSAT5 to do a backward search. They showed that the new algorithm is able to solve a number of real-world benchmarks, at least as fast as other approaches.}
...
an inductive synthesis phase, {\sc synthesise}, and a validation phase, {\sc verify}~\cite{AbateBCCCDKKP17}.  
The two phases interact via a finite set of test vectors, which is updated
incrementally.  Given a specification of the ECPS ({\it e.g.}, stability and safety), the inductive
synthesis procedure tries to find a candidate solution satisfying the
specification for the given set of test inputs.}
%
\textcolor{blue}{If the synthesis phase succeeds in finding a witness, this witness is a
candidate solution to the full synthesis formula.  This candidate
solution is passed to the validation phase, which checks whether it is a proper solution
({\it i.e.}, satisfies the specification for all possible inputs).  If this is the
...
process as follows: (i)~The first verification stage ({\sc safety}) checks that the candidate
solution, which we synthesized to be safe for at least one initial
state, is safe for \emph{all} possible initial states, {\it i.e.}, does not reach
...
iterations, and check that $k{\geq}\overline{k}$}.
