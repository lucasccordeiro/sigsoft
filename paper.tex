...
Counter-Example Guided Inductive Synthesis (CEGIS) is currently one of the the most popular approaches to program synthesis in practice~\cite{David15}. Figures~\ref{Counter-Example-Guided-Inductive-Synthesis} shows the CEGIS approach, which has close connections to algorithmic debugging using counterexamples and abstraction refinement. These connections involve two specific steps: synthesizing a repair in the former case, and synthesizing an abstraction function in the latter.
%
\begin{figure}[h]
	\centering
