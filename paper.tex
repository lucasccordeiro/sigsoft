...
Generally, properties that must hold for ECPSs are related to safety/reachability and liveness. The earliest works on HS and ECPS proposed approaches based on CTL and LTL~\cite{Alur93} to specify properties and verify these systems. In the last decade, the Signal Temporal Logic (STL)~\cite{Maler04,Donze12} was proposed and have been used for this purpose, providing more suitable operators for dynamical systems and signal processing.  Therefore, model-checking procedures have been employed to verify those properties, even under undecidability conditions via approximations, {\it e.g.}, bounding the number of steps from initial states, which leads to bounded model checking (BMC)~\cite{Veanes09}. 
...
An efficient way to synthesize reliable ECPSs is the Counterexample Guided Inductive Synthesis (CEGIS)~\cite{David15}. Its key idea is the definition (by a user) of a set of safe states, {\it i.e.}, the specification of $\Phi$, as well as a generic template for a control system that must satisfy such a specification. The, a CEGIS engine compute values for the unknown symbols of the chosen template, which hold $\Phi$~\cite{RienerKFB16}. Abate~{\it et al.}~\cite{Abate17,abatecav2017} presented a method for synthesizing stable controllers that are suitable to continuous plants given as transfer functions~\cite{Abate17} and state-space representations~\cite{abatecav2017}, which exploits bit-accurate verification of software implemented in digital microcontrollers~\cite{Bessa16,Bessa17,dsv_spin2015}; the controllers must ensure non-fragile stability~\cite{Abate17} and safety~\cite{abatecav2017}. Ravanbakhsh and Sankaranarayanan~\cite{Ravanbakhsh15,Ravanbakhsh16} also employed CEGIS based on Lyapunov function to ensure the robust reach-while-stay property ({\it i.e.}, a safety specification)~\cite{Ravanbakhsh16} and non-zeno behavior~\cite{Ravanbakhsh15}. The CEGIS was also employed to synthesize model predictive controllers (MPC) for a wide varieties of applications, {\it e.g.} heating ventilation and air conditioning (HVAC) systems, autonomous vehicles control, and aircraft electric power system~\cite{Ghosh05,Rahman13}.

CEGIS is an important, but not the unique, example of symbolic control synthesis for ECPS. The main advantage of using symbolic synthesis techniques ({\it e.g.}, SMT- and SAT-based) is the possibility of encoding (via logical formula) reachability/safety specifications on the original concrete systems. The references~\cite{Rungger13,AydinGol14,Holub16,Tabuada08,Zamani17,Zamani14,Zamani13,Zamani12,Alkhatib17,Lesser15,Girard13} are examples of works on symbolic control synthesis for ECPS.

A common procedure for controller synthesis for ECPS is based on finite-state approximations of infinite-states systems, {\it i.e.}, the HA. This approach allows the fully automated synthesis of ECPS control systems that ensure the achievement of complex specifications. The weakness of this approach is that it often produces complex controllers with high implementation cost. To avoid this problem, refined controllers have been proposed aiming to provide some invariant properties for close-loop systems, {\it e.g.}: safety properties~\cite{Dallal17,Dallal13,Girard13}, reachability properties~\cite{Alkhatib17,Habets06}, stability~\cite{Zamani12,Tabuada08}, and safety/reachability properties considering the quantization effects~\cite{Reissig17}.
