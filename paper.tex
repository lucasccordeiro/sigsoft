...
The basic idea of program synthesis is to automatically construct a program $P$ that satisfies a correctness specification $\phi$. In particular, program synthesis is automatically performed by engines that use a correctness specification $\phi$ as the starting point, and then produce a sequence of candidate programs that satisfy $\phi$~\cite{Alur13}. In this respect, a given candidate program $p$ is iteratively refined to match specification $\phi$ more closely. 

Counter-Example Guided Inductive Synthesis (CEGIS) is currently the most popular approach to program synthesis in practice~\cite{David15}. CEGIS has close connections to algorithmic debugging using counterexamples and abstraction refinement. These two connections, however, involve two specific steps: synthesizing a repair in the former case, and synthesizing an abstraction function in the latter.

Nowadays, program synthesis engines that implement Counter-Example Guided Inductive Synthesis (CEGIS)~\cite{sketch} can automatically produce programs for a large variety of specifications due to the combination of automated testing, genetic algorithms, and SMT-based automated reasoning~\cite{Sharma14}.
...
%32
\bibitem{Alur13}
Alur R., Bodík R., Juniwal G., Martin M., Raghothaman M., Seshia S., Singh R., Solar-Lezama A., Torlak E., Udupa A.:
