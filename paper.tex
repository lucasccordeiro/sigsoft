...
\begin{enumerate}
	...
	\item \textcolor{blue}{Provide a form of system evolution, in such a way that a version more adapted to the current scenario is obtained.}
\end{enumerate}
...
\textcolor{blue}{The tenth aspect is related to the current development and test strategies adopted for embedded systems, which aim at simply providing robust structures, instead of antifragile ones. In summary, it is not feasible to account for every scenario or error, but it would be more interesting to provide a module or structure capable of thriving on problems and getting more robust. Such an aspect shares some similarity with the ninth one and may also be achieved through ML techniques.}
...
\textcolor{blue}{Due to the fact that the mentioned research problems are the focus of the present technology area, many works tackled them and other researchers still address them. As a consequence, we can cite the followinf contributions toward them.}
...
\textcolor{blue}{In addition, a aspect closely related to ML techiniques, as mentioned in Section \cite{Verification-Challenges}, is system fragility. Currently, most design strategies try to produce robust systems, which are able to deal with a limited set of common errors that happen when they are operating, {\it i.e., they are robust}. Nonetheless, that is effective only if problems foreseen during design phases happen; otherwise, operation may be compromised.}  
...
\textcolor{blue}{As a result, it may be more interesting to develop methodologies that are not simply robust, regarding a predetermined condition, but instead thrive on problems and grow stronger (as the human body, which gets stronger when exposed to germs), which leads to antifragile systems. In engineering, we are only beginning to develop solutions that can be classified as antifragile, such as multiple-input/multiple-output (MIMO) systems, which use multiple signal copies together with multipath to achieve performance enhancement~\cite{spectrum2013}.}

\textcolor{blue}{Systems that could greatly benefit from being antifragile are the cyber-physical ones, which interact with other external systems and use their internal representations to interpret physical properties. As faults occur, which may be treated with some sort of system elasticity, and external environments are normally dynamic, given that it naturally changes with time, variations in operating conditions happen and should be tolerated, in order to keep behavior quality \cite{antifragile}. Besides, when discussing the fundamentals of antifragility, it is difficult not to tackle machine learning \cite{antifragile}, which has the potential to provide some wisdom regarding correct behavior and internal representations, in order to keep a system working in a volatile environment.}

\textcolor{blue}{For instance, we could think about a system including an adaptation module composed by two elements: a machine learning engine, capable of capturing current behaviors and identify different trens, and a SMT-based verification method, capable of recursively performing retuning procedures, which could test new parameters suggested by the machine learning module and then judge theis suitability. As a consequence, we would be able to design systems with the potential to being antifragile, given that they would learn from new challenges and then modify its behavior model, in order to account for new errors and threats.}
